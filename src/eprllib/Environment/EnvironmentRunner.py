...
class EnvironmentRunner:
    ...
    def get_weather_prediction(
        ...
        if not self.observation_config[agent]['use_one_day_weather_prediction']:
            return {}
        # Get timestep variables that are needed as input for some data_exchange methods.
        num_time_steps_in_hour: int = self.api.exchange.num_time_steps_in_hour(state_argument)
        ...
        for key in prediction_variables.keys():
            if prediction_variables[key]:
                # For each hour for the present and next day and for each timestep.
                for h in range(48):
                    for timestep in range(1, num_time_steps_in_hour+1):
                        if h < 24:
                            variables.update({
                                get_parameter_prediction_name(agent,key,h, timestep): prediction_variables_methods[f'today_weather_{key}_at_time'](state_argument,h,timestep)
                            })
                        else:
                            variables.update({
                                get_parameter_prediction_name(agent,key,h,timestep): prediction_variables_methods[f'tomorrow_weather_{key}_at_time'](state_argument,h-24,timestep)
                            })
        return variables
    ...
    def get_user_occupation_forecast(
        ...
        if self.observation_config[agent]['user_occupation_function']:
            ...
            self.occupancy_next_timestep  = calculate_occupancy(
                self.api.exchange.zone_time_step_number(state_argument),
        ...
        if self.observation_config[agent]['user_occupation_forecast']:
            ...
            forecast_vector = calculate_occupancy_forecast(
                ...
                self.api.exchange.num_time_steps_in_hour(state_argument)
            )

            for h in range(24):
                for timestep in range(1, self.api.exchange.num_time_steps_in_hour(state_argument)+1):
                    variables.update({get_user_occupation_forecast_name(agent,h,timestep): forecast_vector[h][timestep]})
