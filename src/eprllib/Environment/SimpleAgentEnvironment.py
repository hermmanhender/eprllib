...
class SingleAgentEnvironment(gym.Env): # type: ignore
    """
    ...
    runner: EnvironmentRunner = None # type: ignore
    obs_queue: Queue[Dict[str, ObsType]] = None # type: ignore
    act_queue: Queue[ActType] = None # type: ignore
    infos_queue: Optional[Queue[Dict[str, Dict[str, Any]]]] = None
    episode = -1
    timestep = 0
    terminateds = False
    truncateds = False
    last_obs: Dict[str, ObsType] # type: ignore
    ...
    def __init__(
        ...
        assert len(self.possible_agents) == 1, f"SingleAgentEnvironment: At least one agent must be defined in the environment."
        
        logger.info(f"SingleAgentEnvironment: Possible agents: {self.possible_agents}")
        ...
        logger.debug(f"SingleAgentEnvironment: Episode configuration: {self.episode_fn.get_episode_config(self.env_config)}")
        
        # Connector funtion.
        self.connector_fn: BaseConnector = self.env_config['connector_fn'](self.env_config["connector_fn_config"])
        logger.debug(f"SingleAgentEnvironment: Connector configuration: {self.connector_fn.connector_fn_config}")
        ...
        for agent in self.possible_agents:
            ...
            logger.info(f"SingleAgentEnvironment:The ActionMapper function for agent {agent} was initialized.")
            ...
            logger.info(f"SingleAgentEnvironment: The actuator names in ActionMapper function of agent {agent} were updated.")
            action_space.update({agent: self.action_mapper_fn[agent].get_action_space_dim()}) # Asignation of environment action space.
            logger.info(f"SingleAgentEnvironment: The action space of agent {agent} was assignated to the action space.")
            ...
            logger.info(f"SingleAgentEnvironment: The filter of agent {agent} was initialized.")
            ...
            logger.debug(f"SingleAgentEnvironment: Reward function for agent {agent} initialized.")
            
        
        for agent in self.possible_agents:
            # Build the action_space dictionary.
            self.action_space = action_space[agent]
            logger.debug(f"SingleAgentEnvironment: Action space: {self.action_space}")
            
            # asignation of environment observation space.
            self.observation_space = self.connector_fn.get_all_agents_obs_spaces_dict(self.agents)[agent]
            logger.debug(f"SingleAgentEnvironment: Observation space: {self.observation_space}")
        ...
        self.runner: EnvironmentRunner = None  # type: ignore
        self.obs_queue: Queue[Dict[str, ObsType]] = None  # type: ignore
        self.act_queue: Queue[ActType] = None  # type: ignore
        self.infos_queue: Optional[Queue[Dict[str, Dict[str, Any]]]] = None
        ...
        logger.info(f"SingleAgentEnvironment: Output path for EnergyPlus simulation results: {self.output_path}")
        ...
        logger.info("SingleAgentEnvironment: Environment initialization complete")
    ...
    def reset(
        ...
        if not self.truncateds:
            ...
            try:
                ...
                logger.debug(f"SingleAgentEnvironment: Episode configuration: {self.env_config}")
        ...
        single_agent_obs: ObsType = None  # type: ignore
        single_agent_infos: Dict[str, Any] = None  # type: ignore
        for agent in self.agents:
            single_agent_obs = obs[agent] # type: ignore
            single_agent_infos = infos[agent]
        
        assert single_agent_obs is not None, "SingleAgentEnvironment: Observation is None."
        assert single_agent_infos is not None, "SingleAgentEnvironment: Infos is None."
        
        return single_agent_obs, single_agent_infos
    ...
    def step(
        ...
        if self.runner.failed():
            ...
            logger.error("SingleAgentEnvironment: EnergyPlus failed with an error!")
        ...
        if self.runner.simulation_complete:
            ...
            obs = self.last_obs
            infos = self.last_infos
            logger.info(f"SingleAgentEnvironment: Simulation completed after {self.timestep} timesteps.")
        ...
        reward_dict: Dict[str, SupportsFloat] = {}
