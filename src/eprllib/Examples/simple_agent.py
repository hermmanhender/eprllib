...
from eprllib.Utils.examples_utils import add_eprllib_example_script_args

parser = add_eprllib_example_script_args()
...
if __name__ == "__main__":
    ...
    eprllib_config.episodes(
        episode_fn = task1d_cofiguration,
        episode_fn_config = {
    ...
    register_env(name="EPEnv", env_creator=lambda args: BaseEnvironment(args))
    ...
    if not restore:
        ...
        algo.training(
            ...
            lr = 0.0001,#tune.grid_search([0.0001,0.001,0.01]) if tuning else 0.001,#
            ...
            use_critic = True,
            use_gae = True,
            lambda_ = 0.7,#tune.quniform(0.4, 0.9, 0.1) if tuning else 0.7,#
            use_kl_loss = True,
            kl_coeff = 0.2,#tune.quniform(0.1, 1, 0.1) if tuning else 0.2,#
            kl_target = 0.7,#tune.quniform(0.1, 0.9, 0.1) if tuning else 0.7,#
            shuffle_batch_per_epoch = True,
            vf_loss_coeff = 0.9,#tune.quniform(0.1, 1, 0.1) if tuning else 0.9,#
            entropy_coeff = 0.01,#tune.choice([0.,0.1,0.2]) if tuning else 0.1,#
