...
eprllib_config.agents(
    agents_config = {
        "HVAC": AgentSpec(
            ...
            reward = RewardSpec(
                reward_fn = NygardFerguson1990_comfort,
                reward_fn_config = {
...
if not restore:
    ...
    algo.training(
        ...
        gamma = 0.9,#tune.grid_search([0.7,0.9,0.99]) if tuning else 0.8,#
        lr_schedule = [
            (0, 3e-4),
            (1e6, 1e-4),
            (2e6, 5e-5)
            ],#tune.grid_search([0.0001,0.001,0.01]) if tuning else 0.003,#
        train_batch_size = 144*31*8,#tune.grid_search([100,1000,10000,100000]) if tuning else 12961*7,
    ...
    tuner = tune.Tuner(
        ...
        run_config=air.RunConfig(
            ...
            stop = {"info/num_env_steps_trained": 1008 * 100000},
            log_to_file = True,
