...
from eprllib.Agents.Rewards.NygardFerguson1990 import NygardFerguson1990_ended
...
with open("src/eprllib/examples/example_thermostat_files/curriculum_task1-1.json", "r") as f:
...
eprllib_config.agents(
    agents_config = {
        "HVAC": AgentSpec(
            ...
            reward = RewardSpec(
                reward_fn = NygardFerguson1990_ended,
                reward_fn_config = {
...
algo.training(
    ...
    train_batch_size = 289*7*10, # 144 timesteps per episode (one day), 64 episodes.
    minibatch_size = 289*10,
    num_epochs = 10,
    ...
    vf_clip_param = 0.2, #tune.quniform(0.1, 0.3, 0.05),
)
algo.env_runners(
...
tuner = tune.Tuner(
    ...
    run_config=air.RunConfig(
        ...
        stop = {"info/num_env_steps_trained": 10000000},
        log_to_file = True,
