...
class CoraciObsSpaceFilter(BaseFilter):
    ...
    def get_filtered_obs(
        ...
        # 6. Humidity
        # ===========
        site_humidity = normalization_minmax(agent_states_copy[get_variable_name(
            self.agent_name, 
            "Site Outdoor Air Relative Humidity", 
            "Environment"
        )], 0, 100)
        
        tz_himidity = normalization_minmax(agent_states_copy[get_variable_name(
            self.agent_name, 
        ...
        return np.array([to_sin_transformation(actual_hour,0,23),minutes, *self.dt_list, t_o, *t_o_predictions, cooling, heating, occupancy_status, *occupation_prediction, site_humidity, tz_himidity], dtype='float32')
