...
class Coraci2021Connector(BaseConnector):
    ...
    def get_agent_obs_dim(
        ...
        self.obs_indexed[agent] = {
            ...
            f"{agent}: Cooling:DistrictCooling": 3,
            f"{agent}: Heating:DistrictHeatingWater": 4,
            f"{agent}: Zone People Occupant Count: Thermal Zone": 5,
            f"{agent}: User Occupation Forecast +1h": 6,
            f"{agent}: User Occupation Forecast +2h": 7,
            f"{agent}: User Occupation Forecast +3h": 8,
            f"{agent}: User Occupation Forecast +4h": 9,
            f"{agent}: User Occupation Forecast +5h": 10,
            f"{agent}: User Occupation Forecast +6h": 11,
            f"{agent}: Site Outdoor Air Drybulb Temperature: Environment": 12,
        }
