...
def episode_epJSON(env_config: dict):
    ...
    """
    # Modelo de volumen aleatorio
    else:
        # epJSON Model: One model have two opposite windows and the other the windows are ortogonals
        env_config['model'] = np.random.choice([1,2])
        # Se guarda el path al modelo elegido
        env_config['idf'] = env_config['idf_folderpath']+'/model_'+str(env_config['model'])+'.epJSON'
        # Read the epJSON file as epJSON Object (is a way to call it when is trated as a dict in Python)
        with open(env_config['idf']) as file:
            epJSON_object: dict = json.load(file)
        
        # Run Period configuration in the epJSON Object
        run_period(
            epJSON_object = epJSON_object,
            ObjectName = next(iter(epJSON_object['RunPeriod'])),
            day = 1 if env_config['longitud_episodio']==365 else int(np.random.randint(1,(365-int(env_config['longitud_episodio'])))),
            longitud_episodio = int(env_config['longitud_episodio'])
        )
        
        # Orientación del modelo
        rotation_angle = int(np.random.choice([0, 90, 180, 270]))
        epJSON_object["Building"][next(iter(epJSON_object['Building']))]["north_axis"] = rotation_angle
        
        # Volumen y vértices
        env_config['volumen'] = change_vertex_volumen(
            epJSON_object=epJSON_object,
            env_config=env_config,
            h = 2 + np.random.random_sample(),
            w = 3 + 7*np.random.random_sample(),
            l = 3 + 7*np.random.random_sample()
            )
        
        # Tamaño de las ventanas
        env_config['window_area_relation_north'] = 0
        env_config['window_area_relation_west'] = 0
        env_config['window_area_relation_south'] = 0
        env_config['window_area_relation_east'] = 0
        window_name = ['ventana_1','ventana_2']
    
        for window in window_name:
            factor = np.random.random_sample()
            factor = factor+0.01 if factor == 0 else factor
            area_factor = window_size_epJSON(
                epJSON_object=epJSON_object,
                window=window,
                factor_escala=factor,
                env_config=env_config
            )
            # Se calculan los factores de relación de área de cada ventana
            if window == 'ventana_1':
                orientacion_ventana = 180
            else:
                if env_config['model'] == 1:
                    orientacion_ventana = 0
                else:
                    orientacion_ventana = 90
            # Corrección del factor por la rotación del edificio    
            if rotation_angle+orientacion_ventana > 315 and rotation_angle+orientacion_ventana <= 45:
                env_config['window_area_relation_north'] = area_factor
            elif rotation_angle+orientacion_ventana > 45 and rotation_angle+orientacion_ventana <= 135:
                env_config['window_area_relation_east'] = area_factor
            elif rotation_angle+orientacion_ventana > 135 and rotation_angle+orientacion_ventana <= 225:
                env_config['window_area_relation_south'] = area_factor
            else:
                env_config['window_area_relation_west'] = area_factor
            
        # Tipo de construcción de muros, techo y ventanas
        # TODO: Establecer una configuración de tres capas y cambiar los valores de densidad, calor específico y conductividad, como así también los valores ópticos para tener un abanico más amplio de posibilidades.
        surfaces = [key for key in epJSON_object["BuildingSurface:Detailed"].keys()]
        env_config['construction_config'] = int(np.random.randint(0,4))
        
        for surface in surfaces:
            surface_type = epJSON_object["BuildingSurface:Detailed"][surface]['surface_type']
            if surface_type == "Wall":
                construction_name = "mamposteria" if env_config['construction_config'] < 2 else "seco"
                
            elif surface_type == "Roof":
                construction_name = "tradicional" if env_config['construction_config']%2 == 0 else "aislado"
            
            epJSON_object["BuildingSurface:Detailed"][surface]["construction_name"] = construction_name
        
        # Se calcula el factor global de pérdidas U
        env_config['construction_config'] = u_factor(epJSON_object)
        
        # Modificación de la masa térmica interna
        for key in epJSON_object["InternalMass"].keys():
            epJSON_object["InternalMass"][key]["surface_area"] = float(np.random.random_sample()*30 + 5)
        
        # Se calcula la masa térmica total del edificio
        env_config['internal_mass'] = masa_inercial(epJSON_object)
        
        # Se cambia el límite de capacidad máxima del HVAC Ideal
        env_config['E_max'] = np.random.choice([400.0, 1500.0, 3000.0])/1000/6
        for key in epJSON_object["ZoneHVAC:IdealLoadsAirSystem"].keys():
            epJSON_object["ZoneHVAC:IdealLoadsAirSystem"][key]["maximum_sensible_heating_capacity"] = env_config['E_max']
            epJSON_object["ZoneHVAC:IdealLoadsAirSystem"][key]["maximum_total_cooling_capacity"] = env_config['E_max']
